...
2. 編輯 `docs/config.json`，把 `dataUrl` 的 `OWNER/REPO` 換成你的帳號與 repo：
   ```json
   "dataUrl": "https://raw.githubusercontent.com/your-name/your-repo/data/data.json"
   ```
3. Repo **Settings → Pages → Source** 選 **GitHub Actions**。
4. push 到 `main` 後，`.github/workflows/pages.yml` 會自動把 `docs/` 發佈到 Pages。
...
.github/workflows/pages.yml   自動部署 docs/ 到 Pages
